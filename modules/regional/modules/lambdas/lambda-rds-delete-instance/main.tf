############################################################
# Lambda RDS Delete Instance                              🇧🇷
############################################################

data "archive_file" "lambda_rds_delete_instance" {
  type        = "zip"
...
locals {
  lambda_function_name                      = "rds-delete-instance"
...
module "lambda_rds_delete_instance" {
  ...
  description             = "Deletes an RDS instance, tracks status via Step Functions, and creates a final snapshot if needed."
  handler                 = "lambda_function.lambda_handler"
  runtime                 = "python3.13"
  timeout                 = 30
  create_function         = local.create_lambda_function_effective
  create_role             = false
  lambda_role             = module.iam_role_lambda_rds_delete_instance.arn
  create_package          = false
  local_existing_package  = data.archive_file.lambda_rds_delete_instance.output_path
