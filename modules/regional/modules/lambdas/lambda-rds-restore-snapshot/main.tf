data "archive_file" "lambda_rds_restore_snapshot" {
  type        = "zip"
...
locals {
  lambda_function_name                      = "rds-restore-snapshot"
...
module "lambda_rds_restore_snapshot" {
  ...
  description             = "Restores an RDS instance from a snapshot and monitors the status until completion."
  handler                 = "lambda_function.lambda_handler"
  runtime                 = "python3.13"
  timeout                 = 300
  create_function         = local.create_lambda_function_effective
  create_role             = false
  lambda_role             = module.iam_role_lambda_rds_restore_snapshot.arn
  create_package          = false
  local_existing_package  = data.archive_file.lambda_rds_restore_snapshot.output_path
